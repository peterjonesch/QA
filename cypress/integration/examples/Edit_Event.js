...
describe('Edit events', () => {
    ...
    it('Edit and save event', () => {
        ...
        const editData = {
            ...
            postType: 'Ecolympics or Environmental'
        ...
        cy.get('#location').type('cal').wait(1000).type('{downArrow}').type('{enter}').wait(1000).then(($index) => {
            if ($index.val() !== 'California, USA') {

                cy.get('#location').clear().type('cali').wait(1000).type('{downArrow}').type('{enter}'.wait(1000));
            }
        ...
        // fill in description
        cy.get('*[data-cy="description"]').eq(1).type('{selectAll}{del}').type(testData.description)

        // fill in extended description
        cy.get('*[data-cy="extendedDescription"]').type('{selectAll}{del}').type(testData.extendedDescription)
...
cy.get('button[data-cy="next-btn"]').eq(1).click();
        ...
        // select event location
        cy.get('*[data-cy*="location-options"]>.Mui-checked+*').should('have.text','Online'); //data-cy needs to be added to the label because 
